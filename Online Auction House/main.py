...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///bidderdb.sqlite3"
...
def home():
	if "name" in session:
		...
		flash("You might want to log in.")
		return render_template("home.html")
...
def login():
	if request.method == "POST":  # if sign-in form filled in
		...
		if corresp_db_entry:
			if request.form["pw"] == corresp_db_entry.password:
...
def signup():
	if "name" in session:
		...
		if request.method == "POST":
			rf = request.form
			db.session.add(User(rf["nm"], rf["em"], rf["pw"]))
			db.session.commit()
			session["name"] = rf["nm"]
...
def registeritem():
	if "name" in session:
		if request.method == "POST":
			rf = request.form
			db.session.add(Item(rf["nm"], rf["ds"], session["id"]))
			db.session.commit()
...
def registerauction():
	if "name" in session:
		if request.method == "POST":
			...
			db.session.add(Auction(rf["id"], session["id"], rf["sp"], int(rf["du"])))
			db.session.commit()
...
def adminregister():
	if "name" in session:
		if not session["admin"]:
			if request.method == "POST":
				if request.form["sw"].lower() in ["dublin", "baile átha cliath", "baile atha cliath"]:
					User.query.filter_by(user_id=session["id"]).first().is_admin = True
					db.session.commit()
					session["admin"] = True
...
def addcredit():
	if "name" in session:
		if request.method == "POST":
			...
			if amount > 0:
				User.query.filter_by(user_id=session["id"]).first().balance += amount
...
def make_bid(id):
	if request.method == "GET":
		...
		if bid <= auction_obj.current_bid:  # check valid bid amount
		...
		auction = Auction.query.filter_by(auction_id=id).first()
		...
		if last_highest_bidder != None:
			last_highest_bidder.available_balance += auction.current_bid
			notify(last_highest_bidder.user_id, id, "You have been outbid in this auction.")

		User.query.filter_by(user_id=session["id"]).first().available_balance -= bid
		auction.current_bid = bid
		auction.highest_bidder_id = session["id"]
